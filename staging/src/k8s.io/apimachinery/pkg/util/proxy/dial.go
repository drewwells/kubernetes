...
func DialRequest(req *http.Request, transport http.RoundTripper) (net.Conn, error) {
	...
	if proxyURL == nil {
		...
		return DialURLWithoutProxy(req.URL, transport)
	}
	...
	proxyDialConn, err := DialURLWithoutProxy(proxyURL, transport)
	if err != nil {
		return nil, err
	}
...
func DialURLWithoutProxy(url *url.URL, transport http.RoundTripper) (net.Conn, error) {
